...
import { Button, TextField, FormControl, Typography, TableCell, TableRow, TableHead, Table, TableContainer, TableBody, InputLabel, Select, MenuItem, Checkbox, FormControlLabel, Grid } from '@mui/material';
...
const AddProductPage = ({product}) => {
  ...
  const handleCategoryChange = (e) => {
    ...
    setProductData((prevData) => ({ ...prevData, category: selectedCategoryId }));
  };
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={6}>
            <TextField label="Product Name" name="name" value={productData.name} onChange={handleInputChange} fullWidth required />
          </Grid>
          ...
          <Grid item xs={12}>
            <TextField label="Description" name="description" multiline minRows={6} maxRows={10} value={productData.description} onChange={handleInputChange} fullWidth required />
          </Grid>
        </Grid>
        <br/>
        <br/>
                ...
                <TableCell>Total</TableCell>
                  ...
                  <TableCell >
                    <TextField
                      ...
                      value={((parseFloat(productData["price.distributor"])*(100-parseFloat(productData["discount.distributor"]))*(100+parseFloat(productData["salesTax.distributor"])))/10000)}
                      InputProps={{
                        readOnly: true,
                      }}
                      fullWidth
                  ...
                  <TableCell >
                    <TextField
                      label={`Total Dealer`}
                      value={((parseFloat(productData["price.dealer"])*(100-parseFloat(productData["discount.dealer"]))*(100+parseFloat(productData["salesTax.dealer"])))/10000)}
                      InputProps={{
                        readOnly: true,
                      }}
                      fullWidth
                  ...
                  <TableCell >
                    <TextField
                      ...
                      value={((parseFloat(productData["price.contractor"])*(100-parseFloat(productData["discount.contractor"]))*(100+parseFloat(productData["salesTax.contractor"])))/10000)}
                      InputProps={{
                        readOnly: true,
                      }}
                      fullWidth
        ...
        <Grid container spacing={2}>
          <Grid item xs={3}>
            <FormControlLabel
              control={<Checkbox checked={productData.stockAvailable} onChange={handleInputChange} name="stockAvailable" color="primary" />}
              label="Stock Available"
            />
          </Grid>
          <Grid item xs={3}>
            <FormControlLabel
              control={<Checkbox checked={productData.hotProduct} onChange={handleInputChange} name="hotProduct" color="primary" />}
              label="Hot Product"
            />
          </Grid>
          <Grid item xs={3}>
            <TextField label="Index" name="index" value={productData.index} onChange={handleInputChange} fullWidth />
          </Grid>
          <Grid item xs={3}>
            <TextField label="Shipping Cost" name="shippingCost" value={productData.shippingCost} onChange={handleInputChange} fullWidth />
          </Grid>
